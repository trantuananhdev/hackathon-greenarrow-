...
from pipeline.shared.event_inventory import (
    TIMEZONE,
...
def main() -> None:
    ...
    parser.add_argument(
        "--locations", type=Path, default=Path("data/reference/dien_bien_locations.parquet")
    )
    parser.add_argument(
        "--raw", type=Path, default=Path("data/events/raw/desinventar_export.zip")
    )
    parser.add_argument(
        "--output", type=Path, default=Path("data/events/desinventar_events.parquet")
    )
